...
class RetrievalEvaluator:
    ...
    def metadata_consistency(self, results: List[Dict[str, Any]]) -> float:
        ...
        # Extract categories from labor contract metadata
        categories = []
        for result in results:
            # Try covered_employee_group first (most specific)
            if "covered_employee_group" in result:  
                categories.append(result["covered_employee_group"])
            # Then try content_type
            elif "content_type" in result:
                categories.append(result["content_type"])
            # Then try union_name as fallback
            elif "union_name" in result and result["union_name"] != "Not Specified": 
                categories.append(result["union_name"])
