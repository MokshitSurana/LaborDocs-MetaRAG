...
class TFIDFRetriever(BaseRetriever):
    ...
    def _init_vectorizer(self):
        ...
        for chunk_id, metadata in list(self.id_to_metadata.items())[:100]:
            ...
            if "job_titles_mentioned" in metadata:  
                keywords.extend(metadata.get("job_titles_mentioned", []))
                
            if "covered_employee_group" in metadata: 
                keywords.append(metadata.get("covered_employee_group", ""))
            
            if "union_name" in metadata and metadata["union_name"] != "Not Specified":  
                keywords.append(metadata.get("union_name", ""))
            
            if "campus_location" in metadata and metadata["campus_location"] != "Not Specified":  
                keywords.append(metadata.get("campus_location", ""))
