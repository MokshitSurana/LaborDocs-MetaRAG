...
# Fix libraries incompatible 
# TensorFlow 2.20 installed with Keras 3, but transformers hasn't been updated to work with Keras 3
# pip install tf-keras to solve this problem
...
# Set up environment variables (.env file in project root)
## Current Setup: Google AI Studio API/Gemini
GOOGLE_API_KEY=AIza_your_token
...
1. Search for Google AI Studio API Key and sign up
2. Click create API API Key on the right corner
3. Name your key and select/create a project
4. Click create key
5. Copy the token (starts with `AIza...`)
...
- **Content Metadata**: content type, keywords, key provisions, job titles mentioned, wage amounts, percentage increases, contains wage data, contains numerical terms
- **Contract Metadata**: covered employee group, union name, campus location, effective dates, exclusions, action requirements
- **Semantic Metadata**: summary, common queries, related topics
