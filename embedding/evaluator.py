...
class EmbeddingEvaluator:
    ...
    def _metadata_consistency(self, embeddings: np.ndarray, metadata: List[Dict[str, Any]], k: int = 5) -> Tuple[float, Dict[str, float]]:
        ...
        if metadata and "covered_employee_group" in metadata[0]:  
            metadata_fields.append(
                ("covered_employee_group", lambda m: m.get("covered_employee_group", "Unknown"))  
            )
            
        if metadata and "union_name" in metadata[0]:  
            metadata_fields.append(
                ("union_name", lambda m: m.get("union_name", "Not Specified")) 
            )
            
        if metadata and "common_queries" in metadata[0]:  
            metadata_fields.append(
                ("common_query", lambda m: m.get("common_queries", ["Unknown"])[0] if m.get("common_queries") else "Unknown")  # ← CHANGED
            )
