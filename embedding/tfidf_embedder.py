...
class TfidfEmbedder(BaseEmbedder):
    ...
    def _extract_metadata_keywords(self, chunk: Dict[str, Any]) -> str:
        ...
        # Job titles mentioned (25%)
        if "content" in metadata and "job_titles_mentioned" in metadata["content"]:
            keywords.extend(metadata["content"]["job_titles_mentioned"])
        
        # Contract-specific categories (20%) 
        if "contract" in metadata:  
            if "covered_employee_group" in metadata["contract"]:
                keywords.append(metadata["contract"]["covered_employee_group"])
            
            if "union_name" in metadata["contract"] and metadata["contract"]["union_name"] != "Not Specified":
                keywords.append(metadata["contract"]["union_name"])
            
            if "campus_location" in metadata["contract"] and metadata["contract"]["campus_location"] != "Not Specified":
                keywords.append(metadata["contract"]["campus_location"])
        
        # Question keywords (15%)
        if "semantic" in metadata and "common_queries" in metadata["semantic"]: 
            for question in metadata["semantic"]["common_queries"]:
