...
from retrieval.hybrid_metarag_retriever import HybridMetaRAGRetriever
...
def create_retrievers(args) -> Dict[str, Dict[str, Any]]:
    ...
    for chunking_type in args.chunking_types:
        ...
        for retriever_type in args.retrievers:
            ...
            #     elif retriever_type.lower() == "metarag":
            #         # Hybrid MetaRAG: Create ALL 3 embedding variants
                    
            #         embedding_configs = [
            #             ("content", "naive_embedding"),
            #             ("prefix", "prefix_fusion_embedding"),
            #             ("tfidf", "tfidf_embedding")
            #         ]
                    
            #         # Create MetaRAG with EACH embedding type
            #         for embedding_name, embedding_dir_name in embedding_configs:
            #             try:
            #                 retriever = HybridMetaRAGRetriever(
            #                     embedding_dir=args.embedding_dir,
            #                     chunking_type=chunking_type,
            #                     embedding_type=embedding_dir_name,  # Different for each variant
            #                     model_name=args.model,
            #                     top_k=args.top_k,
            #                     rag_k=8,
            #                     kg_k=8,
            ...
            #                 retriever_key = f"MetaRAG-{embedding_name}_{chunking_type}"
                            
            #                 retrievers[retriever_key] = {
            #                     "retriever": retriever,
            #                     "name": f"MetaRAG ({embedding_name} + KG, {chunking_type})",
            #                     "type": "metarag",
            #                     "chunking": chunking_type,
            #                     "base_embedding": embedding_name
            #                 }
                            
            #                 logger.info(f"[+] Created: {retriever_key} (using {embedding_name} embeddings)")
                            
            #             except Exception as e:
            #                 logger.error(f"Error creating metarag retriever with {embedding_name} for {chunking_type}: {e}")
            #                 continue  # Skip this variant, try next one
            except Exception as e:
...
def run_retrieval(retriever_info: Dict[str, Any], queries: Dict[str, str], output_dir: str) -> Dict[str, List[Dict[str, Any]]]:
    ...
    meta_src = getattr(retriever, 'id_to_metadata', 'None')
    if meta_src is None:
        base = getattr(retriever, 'base_retriever', None)
        meta_src = getattr(base, 'id_to_metadata', {}) if base else {}
    # Process queries
    completed_count = 0
    ...
    try:
        for query_id, query_text in queries.items():
            ...
            # Enhance results with missing metadata or text
            for result in retrieval_results:
                chunk_id = result.get("chunk_id")
                if not chunk_id or chunk_id not in retriever.id_to_metadata:
                    continue

                meta = meta_src[chunk_id]

                result.setdefault("text", meta.get("text", ""))
                result.setdefault("document_id", meta.get("document_id", ""))
                result.setdefault("document_name", meta.get("document_name", ""))
    ...
    final_output = {
        "run_info": {
            ...
            "top_k": getattr(retriever, "top_k",
                             getattr(getattr(retriever, "base_retriever", None), "top_k", 5))
        },
        "queries": {
            query_id: {
