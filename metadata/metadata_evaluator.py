...
class MetadataEvaluator:
    ...
    def _calculate_completeness(self, chunks):
        ...
        fields = {
            "content_type": 0,
            "keywords": 0,
            "job_titles_mentioned": 0, 
            "covered_employee_group": 0,  
            "union_name": 0,  
            "campus_location": 0,  
            "summary": 0,
            "common_queries": 0, 
            ...
            if "metadata" in chunk:
                ...
                if "content" in metadata:
                    ...
                    if content.get("keywords") and len(content["keywords"]) > 0:
                        fields["keywords"] += 1
                    if content.get("job_titles_mentioned") and len(content["job_titles_mentioned"]) > 0:
                        fields["job_titles_mentioned"] += 1

                # Contract fields
                if "contract" in metadata:
                    contract = metadata["contract"]
                    if contract.get("covered_employee_group"):
                        fields["covered_employee_group"] += 1
                    if contract.get("union_name") and contract["union_name"] != "Not Specified":
                        fields["union_name"] += 1
                    if contract.get("campus_location") and contract["campus_location"] != "Not Specified":
                        fields["campus_location"] += 1
                ...
                if "semantic" in metadata:
                    ...
                    if semantic.get("summary"):
                        fields["summary"] += 1
                    if semantic.get("common_queries") and len(semantic["common_queries"]) > 0:
                        fields["common_queries"] += 1
    ...
    def _calculate_diversity(self, chunks):
        ...
        for chunk in chunks:
            if "metadata" in chunk:
                ...
                if "contract" in metadata:
                    employee_group = metadata["contract"].get("covered_employee_group")
                    if employee_group:
                        primary_categories.append(employee_group)

                # Common queries (extract key terms)
                if "semantic" in metadata:
                    queries = metadata["semantic"].get("common_queries", [])
                    intents.extend(queries)
        
        # Calculate diversity (unique values)
        unique_employee_groups = len(set(primary_categories)) 
        ...
        return {
            "unique_content_types": unique_employee_groups,
    ...
    def _calculate_intent_coverage(self, chunks):
        ...
        # Define standard content types for labor contracts
        ...
        for chunk in chunks:
            if "metadata" in chunk and "content" in chunk["metadata"]:
                content_type = chunk["metadata"]["content"].get("content_type", {}).get("primary")
                if content_type:
                    all_content_types.add(content_type)

        # Calculate coverage
        covered_types = standard_content_types.intersection(all_content_types)
        coverage_percentage = (len(covered_types) / len(standard_content_types)) * 100 if standard_content_types else 0
    
        return {
            "standard_content_types": list(standard_content_types),
            "covered_content_types": list(covered_types),
