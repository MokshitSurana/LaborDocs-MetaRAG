...
def metadata_consistency(chunks: List[Dict[str, Any]]) -> float:
    ...
    for chunk in chunks:
        ...
        if "covered_employee_group" in chunk:  
            categories.append(chunk["covered_employee_group"])
        # Then try content_type
        elif "content_type" in chunk:
            categories.append(chunk["content_type"])
        # Then try union_name as fallback
        elif "union_name" in chunk and chunk["union_name"] != "Not Specified": 
            categories.append(chunk["union_name"])
